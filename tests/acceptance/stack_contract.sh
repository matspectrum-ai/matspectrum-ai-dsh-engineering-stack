...
require_file 'pnpm-workspace.yaml' 'root pnpm security policy exists'
require_text 'pnpm-workspace.yaml' 'strictDepBuilds: true' 'unreviewed dependency builds fail closed'
require_text 'pnpm-workspace.yaml' 'node-pty: true' 'PTY native build is explicitly approved'
require_text 'pnpm-workspace.yaml' 'koffi: true' 'Koffi native build is explicitly approved'
require_text 'pnpm-workspace.yaml' "'@deepseek-ai/dsh-subprocess-local@0.1.0-rc.6': true" 'DSH subprocess postinstall is version-scoped and approved'
require_text 'pnpm-workspace.yaml' "'@google/genai': false" 'Google GenAI lifecycle scripts are explicitly denied'
require_text 'pnpm-workspace.yaml' 'protobufjs: false' 'protobufjs lifecycle scripts are explicitly denied'
require_file 'pnpm-lock.yaml' 'canonical dependency lockfile exists'
require_text 'pnpm-lock.yaml' "lockfileVersion: '9.0'" 'lockfile schema is pinned to pnpm lockfile v9'
require_text 'pnpm-lock.yaml' 'specifier: 0.1.0-rc.6' 'lockfile importer pins DSH rc.6'
...
require_file '.github/workflows/foundation.yml' 'foundation CI workflow exists'
require_text '.github/workflows/foundation.yml' 'pnpm install --frozen-lockfile' 'CI installs exclusively from the canonical lockfile'
